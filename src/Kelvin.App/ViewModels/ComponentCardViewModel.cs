...
public partial class ComponentCardViewModel : ObservableObject
{
    ...
    public ComponentCardViewModel(ComponentReading first)
    {
        ...
        Limit = first.ThrottleLimitC ?? DefaultScale(first.Kind);
    }

    private static double DefaultScale(ComponentKind kind) => kind switch
    {
    ...
    public void Update(ComponentReading r, double? ambientC)
    {
        ...
        if (r.HotspotC is { } h) meta.Add($"hotspot {h:0}°");
        ...
        DeltaText = r.TemperatureC is { } temp && ambientC is { } amb
            ? $"Δ {temp - amb:+0.#;-0.#}° vs outside"
            : "";
    }
}
