...
/// <summary>Hand-drawn time-series chart in the Kelvin dialect: min/max band,
...
public sealed class TimeSeriesChart : FrameworkElement
{
    ...
    private const double MarginLeft = 42, MarginRight = 12, MarginTop = 12, MarginBottom = 26;
    ...
    protected override void OnRender(DrawingContext dc)
    {
        ...
        var gridPen = new Pen(new SolidColorBrush(Color.FromRgb(0x16, 0x1D, 0x27)), 1);
        gridPen.Freeze();
        ...
        for (double v = yMin; v <= yMax + 0.01; v += step)
        {
            ...
            var txt = new FormattedText($"{v:0}°", CultureInfo.InvariantCulture, FlowDirection.LeftToRight,
                new Typeface(Mono, FontStyles.Normal, FontWeights.Normal, FontStretches.Normal), 10, labelBrush, dip);
        ...
        band.Freeze();
        var bandBrush = new SolidColorBrush(Color.FromArgb(28, 0x38, 0xBD, 0xF8));
        bandBrush.Freeze();
        ...
        {
            var ambPen = new Pen(new SolidColorBrush(Color.FromArgb(150, 0x8B, 0x98, 0xA5)), 1.3)
        ...
        // Average line.
        var linePen = new Pen(new SolidColorBrush(ThermalPalette.Accent), 1.8) { LineJoin = PenLineJoin.Round };
        linePen.Freeze();
        ...
        if (Markers is { Count: > 0 } markers)
        {
            foreach (ChartMarker m in markers)
            {
                ...
                dc.DrawText(txt, new Point(Math.Clamp(x - txt.Width / 2, 0, w - txt.Width), MarginTop - 10));
            }
        }
        ...
        if (_hover is { } hover && hover.X >= MarginLeft && hover.X <= w - MarginRight)
        {
            ...
            var crossPen = new Pen(new SolidColorBrush(Color.FromArgb(70, 0xE8, 0xEE, 0xF4)), 1);
            crossPen.Freeze();
            dc.DrawLine(crossPen, new Point(x, MarginTop), new Point(x, h - MarginBottom));
            dc.DrawEllipse(new SolidColorBrush(ThermalPalette.Accent), null, new Point(x, Y(nearest.Avg)), 3, 3);
            ...
            var boxBrush = new SolidColorBrush(Color.FromArgb(235, 0x16, 0x1D, 0x27));
            var boxPen = new Pen(new SolidColorBrush(Color.FromRgb(0x1F, 0x28, 0x33)), 1);
            dc.DrawRoundedRectangle(boxBrush, boxPen, new Rect(bx, by, boxW, boxH), 7, 7);
